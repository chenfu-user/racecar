...
int main(int argc, char** argv)//初始化amclnode类对象
{
        ...
        first_reconfigure_call_(true)
{
  ...
  private_nh_.param("resample_interval", resample_interval_,2);//原2,一个激光雷达，每次都更新
  double tmp_tol;
...
AmclNode::laserReceived(const sensor_msgs::LaserScanConstPtr& laser_scan)
{
  ...
  if(pf_init_)
  {
    ...
    m_force_update=true;// 为无运动时是否更新(更改）
  ...
  else if(pf_init_ && lasers_update_[laser_index])
  {
    ...
    // Pose at last filter update当前每一个点的先验位姿
    //this->pf_odom_pose = pose;
  }

  bool resampled = true;//更改
  // If the robot has moved, update the filter
  if(lasers_update_[laser_index])
  {//对原始激光雷达数据进行处理，转换到amcllaserdata
    AMCLLaserData ldata;
    ...
    lasers_update_[laser_index] = true;//更改
  ...
  {
    // Read out the current hypotheses计算得到权值最高的位姿，重新发布map与odom之间的转换
...
//move_base减去odom_base得到map_odom，并发布
